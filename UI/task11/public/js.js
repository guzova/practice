let data = (function () {
    ...
    function removeArticle(id) {
        ...
        if (typeof id === 'string') {
            for (let i = 0; i < articles.length; i++) {
                if (id === articles[i].id) {
                    httpModel.addArticleBin(articles[i]);
                    articles.splice(i, 1);
...
function authorize(author) {
    showMainPage();
    showButthons();
    document.getElementById('authorize').innerText = 'Quit';
...
function filter() {
    ...
    let author = document.querySelector('input.nameAuthor').value;
